...
// ТУТ ВСТАВЬ СВОЙ КЛЮЧ: https://steamcommunity.com
...
passport.use(new SteamStrategy({
    ...
    if (!users[sid]) users[sid] = { id: sid, name: profile.displayName, avatar: profile._json.avatarfull, balance: 1000, inventory: [] };
...
app.post('/open', (req, res) => {
    ...
    req.user.inventory.push({ ...win, iid: Date.now() });
    liveDrops.unshift({ img: win.img });
    if (liveDrops.length > 12) liveDrops.pop();
    res.json({ win, balance: req.user.balance });
});
